                                  ...
                                  const otIcmp6Header * p_icmp_header)
{
    ...
    {
        bsp_thread_ping_indication_set();
    }
}
#endif // LEDS_NUMBER > 0 && !(defined BSP_SIMPLE)
...
static void handle_role_change(void * p_context, otDeviceRole role)
{
    ...
    switch(role)
    {
        ...
        case OT_DEVICE_ROLE_LEADER:
            bsp_thread_indication_set(BSP_INDICATE_THREAD_CONNECTED);
            break;

        case OT_DEVICE_ROLE_DISABLED:
            bsp_thread_indication_set(BSP_INDICATE_THREAD_DISABLED);
            break;

        case OT_DEVICE_ROLE_DETACHED:
        default:
            bsp_thread_indication_set(BSP_INDICATE_THREAD_DETACHED);
            break;
    }
}
...
ret_code_t bsp_thread_init(otInstance * p_instance)
{
    ...
    assert(otSetStateChangedCallback(p_instance, &state_changed_callback, p_instance) ==
           OT_ERROR_NONE);
    assert(otIcmp6RegisterHandler(p_instance, &m_icmp6_handler) == OT_ERROR_NONE);
#endif // LEDS_NUMBER > 0 && !(defined BSP_SIMPLE)
